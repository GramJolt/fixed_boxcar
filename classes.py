...
class car:
	...
	def make_car_from_gene(self, gene):
		# gen[19]=タイヤの数(1~8個の間で) ***

		if(setting.NO_WHEEL == True):
			check_num = round(gene[19], 3) * 100
			check_range_bot = 33
			check_range_mid = 66
			if 0<=check_num<check_range_bot:
				wheel_num = 0
			elif check_range_bot <= check_num <= check_range_mid:
				wheel_num = 1
			else:
				wheel_num = 2
...
class do_stuff():
	...
	def draw_any(self):
		...
		#ログフォルダの作成
		if(os.path.exists(setting.filename)==False):
			os.mkdir(setting.filename)
		...
		while running and self.gen <= setting.MAX_GEN:
			...
			if self.killed == self.population_size:
				...
				#全個体の評価値ログファイルの作成
				from mklog import create_log
				create_log(self.population_data, self.gen, self.population_size, setting.SORT_VALUE)
				...
				#各世代の最良解をログにとる
				with open('{}{}'.format(setting.curdir + '/', setting.filename) + '/' + setting.filename1,'a') as f:
